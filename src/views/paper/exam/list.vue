<template>
  ...
  <div>
    ...
    <div v-if="breakShow" style=" cursor: pointer; padding: 20px 20px 0px 20px" @click="toExam">
      <el-alert
        :closable="false"
        title="您有正在进行的错题测验，离线太久将被作废哦，点击此处可继续！"
        type="error"
      />
    </div>
    ...
    >
      <template slot="filter-content">
        ...
        <el-input v-model="listQuery.params.title" placeholder="搜索名称" style="width: 200px;" class="filter-item" />
      ...
      <template slot="data-columns">
        ...
        >
          <template slot-scope="scope">
            <el-button v-if="scope.row.state===0" icon="el-icon-caret-right" type="primary" size="mini" @click="handlePre(scope.row.id)">开始重做</el-button>
